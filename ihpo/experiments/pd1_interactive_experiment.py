...
class PD1InteractiveExperiment(BenchmarkExperiment):
    ...
    def evaluate_config(self, cfg, budget=None):
        ...
        if budget is not None:
            warnings.warn('PD1 Benchmark does not support multiple fidelities.')
    ...
    def get_bopro_intervention(self):
        ...
        for k, v in interaction_spec.items():
            ...
            k = k.replace('.', '-') # BOPRO cannot handle dots in HP names
    ...
    def setup_bopro_json(self, benchmark):
        ...
        transformed_bopor_search_space = {}
        for k, v in borpo_search_space.items():
            k = k.replace('.', '-')
            transformed_bopor_search_space[k] = v
        json_dict = {
            ...
            "input_parameters": transformed_bopor_search_space,
        ...
        self._bopro_file_name =  './bopro_experiments/pd1_interactive.json'
